...
extension PointRepositoryTest {
    func test_포인트수령여부조회_정상적인변환() {
        // Given
        let expectedData: PointDetail = .init(point: 100, period: 100, pointStatuses: [.stub, .stub, .stub])
        mockService.getPointListResult = Just(.stub)
    ...
    func test_포인트수령여부조회_네트워크에러발생시_에러반환() {
        // Given
        mockService.getPointListResult = Fail(error: HMHNetworkError.testErr)
            .eraseToAnyPublisher()
