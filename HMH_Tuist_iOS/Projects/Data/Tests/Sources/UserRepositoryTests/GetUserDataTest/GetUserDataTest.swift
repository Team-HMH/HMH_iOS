...
/// 유저 정보 불러오기 API  데이터 변환 테스트
extension UserRepositoryTests {
    func test_유저정보불러오기_정상적인변환() {
        
        let testCases = Array(zip(User.expectedData, UserResult.resultData))
        let expectation = XCTestExpectation(description: "유저 정보 불러오기 레포지토리 변환이 정상적으로 성공했습니다!")
        expectation.expectedFulfillmentCount = testCases.count
        
        for (expectedData, resultData) in testCases {
            mockService.getUserDataResult = Just(resultData)
            ...
            sut.getUserData()
                .sink(receiveCompletion: { completion in
                    if case .failure(let error) = completion {
                        XCTFail("유저 정보 불러오기 API 변환 중 실패했습니다: 에러 \(error)")
                    }
    ...
    func test_유저정보불러오기_네트워크에러발생시_에러반환() {
        ...
        for expected in testCases {
            mockService.getUserDataResult = Fail(error: expected).eraseToAnyPublisher()
